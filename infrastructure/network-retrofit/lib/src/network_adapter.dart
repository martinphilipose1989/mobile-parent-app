...
class NetworkAdapter implements NetworkPort {
  ...
  @override
  Future<Either<NetworkError, StaffListResponseModel>> getStaffList({required GetStaffListUseCaseParams params}) async{
    final response = await safeApiCall(
transportService.getStaffList(schoolId: params.schoolId, platform: params.platform)
    );
  ...
  @override
  Future<Either<NetworkError, FetchStopLogsModel>> fetchStopLogs({required int routeId, required String platform}) async{
    final response = await safeApiCall(
      transportService.fetchStopLogs(routeId: routeId, platform: platform, ),
    );
