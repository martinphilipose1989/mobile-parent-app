...
class RegistrationsDetailsPageView
          ...
          Future.wait(mdmAttributeList).then((_) {
            ...
            if (model.showWidget.value == 1) {
              model.fetchAllDetails(
                  enquiryDetailArgs?.enquiryId ?? '', "ParentInfo");
            }
            if (model.showWidget.value == 2) {
              model.fetchAllDetails(
                  enquiryDetailArgs?.enquiryId ?? '', "ContactInfo");
            }
            if (model.showWidget.value == 3) {
              model.fetchAllDetails(
                  enquiryDetailArgs?.enquiryId ?? '', "MedicalInfo");
            }
            if (model.showWidget.value == 4) {
              model.fetchAllDetails(
                  enquiryDetailArgs?.enquiryId ?? '', "BankInfo");
            }
                ...
                child: Column(
                  ...
                  children: [
                    ListItem(
                      ...
                      compeletion: enquiryDetailArgs
                                  ?.formCompletionPercentage ==
                              null
                          ? ''
                          : "${enquiryDetailArgs?.formCompletionPercentage ?? 0}% Completed",
  ...
  Widget registrationsEditingWidgetAsPerIndex(
      int index, RegistrationsDetailsViewModel model, BuildContext context) {
    switch (index) {
      case 0:
        return getEnquiryDetails(enquiryDetailArgs ?? EnquiryDetailArgs(),
            model, model.editRegistrationDetails.value);
      case 1:
            ...
            onData: (value) {
              if (value.status == Status.success) {
                value.data?.data?.enquiryDocuments?.forEach((element) {
                  model.isDocumentUploaded.add(ValueNotifier(
                      (element.file ?? '').isNotEmpty || element.file != null));
                });
              }
            },
  ...
  Widget registrationsWidgetAsPerIndex(
      int index, RegistrationsDetailsViewModel model) {
    switch (index) {
      case 0:
        return getEnquiryDetails(
            enquiryDetailArgs!, model, model.editRegistrationDetails.value);
      case 1:
        return AppStreamBuilder<Resource<ParentInfo>>(
          stream: model.parentDetail,
          initialData: Resource.none(),
        ...
        return AppStreamBuilder<Resource<IVTDetail>>(
            ...
            dataBuilder: (context, result) {
              switch (result?.status) {
                ...
                case Status.success:
                  return (isEditing)
                      ? EnquiryAndStudentEditing(
                          model: model,
                          enquiryDetailArgs: enquiryDetailArgs,
                        )
                      : EnquiryAndStudentDetails(
                          enquiryDetailArgs: enquiryDetailArgs,
                          ivtDetail: result?.data);
                case Status.error:
                  return const Center(
                    child: Text('Enquiries not found'),
                  );
