                      ...
                      children: [
                        ...
                        CustomDropdownButton(
                          ...
                          onSingleSelect: (selectedValue) {
                            if(model.selectedKidsClubType.value == selectedValue){
                              return;
                            }
                            model.selectedKidsClubType.value = selectedValue;
                            model.feeSubTypeId = model.kidsClubEnrollmentDetail.value.data?.feeSubType?.firstWhereOrNull(
                                (element) => element.feeSubType == selectedValue,
                              )?.feeSubTypeId??0;
                            List<String> options = [];
                            (model.kidsClubEnrollmentDetail.value.data?.batches ?? []).forEach((element) {
                        ...
                        AppStreamBuilder<List<String>>(
                          ...
                          dataBuilder: (context, snapshot) {
                            return CustomDropdownButton(
                              ...
                              showBorderColor: false,
                              onMultiSelect: (List<String> selectedValues) {},
                              singleSelectItemSubject: model.selectedMonth,
                              onSingleSelect: (selectedValue) {
                                if(model.selectedMonth.value == selectedValue){
                                  return;
                                }
                                model.selectedMonth.value = selectedValue;
                                model.batchId = model.kidsClubEnrollmentDetail.value.data?.batches?.firstWhereOrNull(
                                  (element) => element.feeSubType == model.selectedKidsClubType.value && element.batchName == selectedValue,
                                  )?.batchId??0;
                                List<String> options = [];
                                (model.kidsClubEnrollmentDetail.value.data?.periodOfService ?? []).forEach((element) {
                                  if (element.feeSubType == model.selectedKidsClubType.value &&
                                      element.batchName == selectedValue) {
                                    if(!options.contains(element.periodOfService)){
                          ...
                          dataBuilder: (context, snapshot) {
                            return CustomDropdownButton(
                              ...
                              onSingleSelect: (selectedValue) {
                                ...
                                model.periodOfServiceId = model.kidsClubEnrollmentDetail.value.data?.periodOfService?.firstWhereOrNull(
                                  (element) => element.feeSubType == model.selectedKidsClubType.value && element.batchName == model.selectedMonth.value &&
                                    element.periodOfService == selectedValue,
                                  )?.periodOfServiceId??0;
                                List<String> options = [];
                                (model.kidsClubEnrollmentDetail.value.data?.feeCategory ?? []).forEach((element) {
                                  if (element.feeSubType == model.selectedKidsClubType.value &&
                                    element.periodOfService == model.selectedPeriodOfService.value
                                  ) {
                                    if(!options.contains(element.feeCategory)){
                                      options.add(element.periodOfService ?? '');
                                    }
                                  }
                                });
                        ...
                        AppStreamBuilder<List<String>>(
                          ...
                          dataBuilder: (context, snapshot) {
                            return CustomDropdownButton(
                              ...
                              showBorderColor: false,
                              onMultiSelect: (List<String> selectedValues) {},
                              singleSelectItemSubject: model.selectedCafeteriaOptFor,
                              onSingleSelect: (selectedValue) {
                                if(model.selectedCafeteriaOptFor.value == selectedValue){
                                  return;
                                }
                                model.feeCategoryId = model.kidsClubEnrollmentDetail.value.data?.feeCategory?.firstWhereOrNull(
                                  (element) => element.feeSubType == model.selectedKidsClubType.value && element.periodOfService == model.selectedPeriodOfService.value,
                                  )?.feeCategoryId??0;
                                model.selectedCafeteriaOptFor.value = selectedValue;
                                if(model.fee.value.isNotEmpty){
                                  model.fee.value = '';
                                }
                              },
