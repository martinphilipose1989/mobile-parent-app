...
class KidsClubViewModel extends BasePageViewModel {
  ...
  BehaviorSubject<KidsClubEnrollmentResponseModel> kidsClubEnrollmentDetail = BehaviorSubject();
  ...
  Future<void> getKidsClubDetail() async {
    exceptionHandlerBinder.handle(block: () {
      GetKidsClubEnrollmentDetailUsecaseParams params = GetKidsClubEnrollmentDetailUsecaseParams(
      ...
      RequestManager<KidsClubEnrollmentResponseModel>(
        params,
        createCall: () => getKidsClubEnrollmentDetailUsecase.execute(params: params))
          .asFlow()
          .listen((event) {
        _fetchKidsClubEnrollmentDetail.add(event);
        if (event.status == Status.success) {
          kidsClubEnrollmentDetail.add(event.data ?? KidsClubEnrollmentResponseModel());
      ...
      RequestManager<VasOptionResponse>(
        ...
        if(event.status == Status.success){
          ...
          ScaffoldMessenger.of(navigatorKey.currentContext!).showSnackBar(const SnackBar(
            content: CommonText(text: "Kids Club Details added successfully",)));
        }
        if(event.status == Status.error){
          showLoader.value = false;
          print("Error");
