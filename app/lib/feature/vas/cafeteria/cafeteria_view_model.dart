...
class CafeteriaDetailViewModel extends BasePageViewModel {
  ...
  BehaviorSubject<CafeteriaEnrollmentResponseModel> cafeteriaEnrollmentDetail = BehaviorSubject();

  final PublishSubject<Resource<CafeteriaEnrollmentResponseModel>> _fetchCafeteriaEnrollmentDetail = PublishSubject();

  Stream<Resource<CafeteriaEnrollmentResponseModel>> get fetchCafeteriaEnrollmentDetail => _fetchCafeteriaEnrollmentDetail.stream;
  ...
  Future<void> getCafeteriaDetail() async{
    exceptionHandlerBinder.handle(block: (){
      GetCafeteriaEnrollmentDetailUsecaseParams params = GetCafeteriaEnrollmentDetailUsecaseParams(
      ...
      RequestManager<CafeteriaEnrollmentResponseModel>(
        params, 
        createCall: ()=> getCafeteriaEnrollmentDetailUsecase.execute(params: params)
      ).asFlow().listen((event){
        _fetchCafeteriaEnrollmentDetail.add(event);
        if(event.status == Status.success){
          cafeteriaEnrollmentDetail.add(event.data?? CafeteriaEnrollmentResponseModel());
    ...
    exceptionHandlerBinder.handle(block: (){
      AddVasDetailUsecaseParams params = AddVasDetailUsecaseParams(
        vasEnrollmentRequest: VasEnrollmentRequest(
          cafeteriaAmount: int.parse(fee.value),
          cafeteriaOptFor: feeSubTypeID,
          cafeteriaPeriodOfService: periodOfServiceID
        ),
      ...
      RequestManager<VasOptionResponse>(
        ...
        if(event.status == Status.success){
          ...
          ScaffoldMessenger.of(navigatorKey.currentContext!).showSnackBar(const SnackBar(
            content: CommonText(text: "Cafeteria Details added successfully",)));
        }
        if(event.status == Status.error){
          showLoader.value = false;
          flutterToastErrorPresenter.show(
