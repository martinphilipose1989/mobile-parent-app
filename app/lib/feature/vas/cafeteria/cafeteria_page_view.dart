    ...
    return AppStreamBuilder<Resource<CafeteriaEnrollmentResponseModel>>(
          ...
          dataBuilder: (context, data) {
            ...
            if(data?.status == Status.success){
                                  ...
                                  (index) {
                                    return CommonRadioButtonWidget<String>(
                                      ...
                                      onOptionSelected: (value){
                                        List<String> options = [];
                                        model.feeSubTypeID = model.cafeteriaEnrollmentDetail.value.data?.feeCategory?.firstWhereOrNull((element)=> (element.feeCategory??'') == value)?.feeCategoryId??0;
                                         (model.cafeteriaEnrollmentDetail.value.data?.periodOfService??[]).forEach((element){
                                          if(element.feeCategory == value){
                                            options.add(element.periodOfService ?? '');
                                          }
                                         });
                                         if(model.radioButtonFeeOption.selectedItem != value){
                                          if((model.radioButtonTerm.selectedItem??'').isNotEmpty){
                                    ...
                                    children: List.generate(
                                      ...
                                      (index) {
                                        return CommonRadioButtonWidget(
                                          ...
                                          onOptionSelected: (value){
                                            model.periodOfServiceID = model.cafeteriaEnrollmentDetail.value.data?.periodOfService?.firstWhereOrNull(
                                              (element)=> ((element.feeCategory??'') ==  model.radioButtonFeeOption.selectedItem && (element.periodOfService??'') == value) 
                                            )?.periodOfServiceId??0;
                                            if(model.fee.value.isNotEmpty){
                                              model.fee.value = "";
                                            }
                                  ...
                                  return model.fee.value.isEmpty ? Center(
                                    ...
                                    children: [
                                      ...
                                      Expanded(
                                        ...
                                        child: CommonElevatedButton(
                                          onPressed: () {
                                            model.radioButtonFeeOption.selectItem("");
                                            model.radioButtonTerm.selectItem("");
              ...
              return const Center(child: CommonText(text: "Data not found"),);
            }
          }
    );
