...
class CancelSchoolTourPageModel extends BasePageViewModel {
  ...
  CancelSchoolTourPageModel(
      this.exceptionHandlerBinder,
      this.cancelSchoolVisitUsecase,
  ...
  final PublishSubject<Resource<SchoolVisitDetail>> schoolVisitDetail =
      PublishSubject();
  final PublishSubject<Resource<SchoolVisitDetailBase>> _cancelSchoolTour =
      PublishSubject();
  Stream<Resource<SchoolVisitDetailBase>> get cacnelSchoolTour =>
      _cancelSchoolTour.stream;
  SchoolVisitDetail? schoolVisitDetailData;
  TextEditingController controller = TextEditingController();
  DateFormat dateFormat = DateFormat('d MMM yyyy');
  final formKey = GlobalKey<FormState>();

  Future<void> cacnelSchoolVisit(
      {required String enquiryID, required String schoolVisitID}) async {
      ...
      RequestManager<MdmAttributeBaseModel>(
        ...
        if (result.status == Status.success) {
          ...
          result.data?.data?.forEach(
              (element) => reasonTypes.add(element.attributes?.reason ?? ''));
        }
        if (result.status == Status.error) {
          flutterToastErrorPresenter.show(
              result.dealSafeAppError!.throwable,
  ...
  List<String> reasonTypes = ['Not Available', 'No Show', 'Not Interested'];
}
