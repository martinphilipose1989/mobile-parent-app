  ...
  Widget getPsaDetails({PSADetail? psaDetail}) {
    return Column(
      children: [
        ...
        _detailItem(
            title: "Parent Type", subtitle: psaDetail?.enquirerParent ?? ''),
        _detailItem(
            title: "Global Id",
            subtitle: psaDetail?.enquirerParent == "Father"
                ? psaDetail?.parentDetails?.fatherDetails?.globalId ?? ""
                : psaDetail?.parentDetails?.motherDetails?.globalId ?? ""),
        _detailItem(
  ...
  Widget getAdmissionDetails({NewAdmissionDetail? newAdmissionDetail}) {
    return Column(
      children: [
        ...
        _detailItem(
            title: "School Location",
            subtitle: newAdmissionDetail?.schoolLocation?.value ?? ''),
        _detailItem(
            title: "Student First Name",
            subtitle: newAdmissionDetail?.studentDetails?.firstName ?? ''),
        _detailItem(
            title: "Student Last name",
            subtitle: newAdmissionDetail?.studentDetails?.lastName ?? ''),
        _detailItem(
            title: "Grade",
            subtitle: newAdmissionDetail?.studentDetails?.grade?.value ?? ''),
        _detailItem(
            title: "Brand", subtitle: enquiryDetailArgs?.brandName ?? ''),
        _detailItem(
            title: "DOB",
            subtitle: newAdmissionDetail?.studentDetails?.dob ?? ''),
        _detailItem(
            title: "Gender",
            subtitle: newAdmissionDetail?.studentDetails?.gender?.value ?? ''),
        _detailItem(
            title: "Existing School Name",
            subtitle: newAdmissionDetail?.existingSchoolDetails?.name ?? ''),
        _detailItem(
            title: "Existing school board",
            subtitle:
                newAdmissionDetail?.existingSchoolDetails?.board?.value ?? ''),
        _detailItem(
            title: "Existing school Grade",
            subtitle:
                newAdmissionDetail?.existingSchoolDetails?.grade?.value ?? ''),
        const SizedBox(
          height: 10,
        ),
        Container(
  ...
  Widget getIvtDetails({IVTDetail? ivtDetail}) {
    return Column(
      children: [
        ...
        _detailItem(
            title: "Parent Type", subtitle: ivtDetail?.enquirerParent ?? ''),
        _detailItem(
            title: "Global Id",
            subtitle: ivtDetail?.enquirerParent == "Father"
                ? ivtDetail?.parentDetails?.fatherDetails?.globalId ?? ""
                : ivtDetail?.parentDetails?.motherDetails?.globalId ?? ""),
        _detailItem(
            title: "Parent First Name",
            subtitle: ivtDetail?.enquirerParent == "Father"
